...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, position_ids, segment_ids,
                                                        speaker_ids)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if position_ids is None:
            ...
            segment_ids = segment_ids.unsqueeze(0).view(-1, seq_length)
            speaker_ids = speaker_ids.unsqueeze(0).view(-1, seq_length)
        else:
            position_ids = position_ids.view(-1, seq_length).long()
            segment_ids = segment_ids.view(-1, seq_length).long()
            speaker_ids = speaker_ids.view(-1, seq_length).long()
